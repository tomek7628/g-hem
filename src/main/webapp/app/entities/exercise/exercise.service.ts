...
export class ExerciseService {
  ...
  protected convertDateFromClient(exercise: IExercise): IExercise {
    const copy: IExercise = Object.assign({}, exercise, {
      modified: exercise.modified && exercise.modified.isValid() ? exercise.modified.format(DATE_FORMAT) : undefined
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((exercise: IExercise) => {
        exercise.modified = exercise.modified ? moment(exercise.modified) : undefined;
      });
    }
    return res;
  }
